...
Last updated: 2026-09-14 (**WGS sibling pipeline, Phase 4 - variant calling: GATK4 HaplotypeCaller
(GVCF mode) + GenotypeGVCFs, the final real analysis stage of this build** - real findings include a
genuinely serious data-corruption bug (HaplotypeCaller and GenotypeGVCFs silently sharing one output
filename, emptying the real GVCF through a symlink), a GATK-specific fasta-extension requirement fixed with
a new local module, and a missing-read-group bug that broke GVCF mode outright. Verified with a real called
variant (NC_007795.1:89 C>T). See "WGS sibling pipeline, Phase 4" below (KNOWN_ISSUES entry 25).)

Last updated: 2026-09-14 (**WGS sibling pipeline, Phase 3 - pre-flight species/contamination
screening: Kraken2 (reused from metagenomics) + Mash Screen + seqkit stats, both Kraken2 and Mash
independently toggleable by owner request** - real findings include a genuine mash/sketch design mistake
found only by running it, a real MultiQC-guard bug from Mash having zero report contribution, and a
concerning real 6GB peak-RAM measurement for Mash screening the real RefSeq DB. See "WGS sibling pipeline,
Phase 3" below (KNOWN_ISSUES entry 24).)

Last updated: 2026-09-14 (**WGS sibling pipeline, Phase 2 - BWA-MEM2 reference-guided alignment +
samtools stats, the first REAL tool stage (Phase 1 was plumbing only)** - `skip_bwamem2`/
`wgs_reference_fasta` params, `BWAMEM2_INDEX`/`BWAMEM2_MEM`/`SAMTOOLS_STATS` wired into `workflows/wgs.nf`,
real measured resource usage recorded (still on a mechanics-only toy fixture, honestly labeled as such - a
real bacterial-genome-scale reference would need far more, per BWA-MEM2's own `28N GB` documented memory
formula). See "WGS sibling pipeline, Phase 2" below.)

Last updated: 2026-09-14 (**WGS sibling pipeline, Phase 1 of an in-progress rollout - plumbing
only, no real tool stages yet, and deliberately NOT reported as more finished than that** - `params.pipeline`
selector added to `main.nf`, new `workflows/wgs.nf`/`conf/modules_wgs.config`/`conf/test_wgs.config`/
`assets/schema_input_wgs.json`. See "WGS sibling pipeline - Phase 1 plumbing" below for the real finding hit
along the way and why this phase intentionally FAILS rather than fakes a pass.)
...
10. **Phase 5's final full `--tag basic` regression re-run (WGS sibling pipeline, PLAN.md §6.9) did not
    finish before being deferred, 2026-09-14** - owner: "seems like its taking too long... note down that we
    need to do this later, when i moved to a better pc." Not a failure - the run was still genuinely
    progressing (verified via `ps`/`free -h` mid-run, memory healthy) when deferred, it was just slow on this
    dev machine's constrained resources, consistent with the "real testing happens on AWS, local RAM limits
    are expected" scope already agreed earlier in §6.9. **What IS actually verified, not deferred**: every
    individual piece this regression run would have re-confirmed already passed earlier the same session -
    the full metagenomics `basic` suite (unchanged by this phase), the two Phase 4 GATK4 `basic` tests, and
    the new Phase 5 combined-everything test (`requires_db`-tagged, real content across all five WGS stages
    confirmed together) all passed in separate runs before this one was started. This final pass was a
    repeat confirmation, not exercising anything new - low risk to defer, but the confirmation itself is
    real work not yet done, not silently assumed. **Pick this up on a more capable machine**: `nf-test test
    tests/main.nf.test --tag basic --profile test,docker` (full suite, ~15+ minutes on this dev machine).
11. **`server/main.py`'s `_pid_alive` (Phase 2 live status, entry 26) is vulnerable to PID reuse after a hard
    restart (power loss, crash, `docker`/WSL2 forced shutdown).** Raised by the owner 2026-09-14 while
    discussing the zombie-process bug this same function was just fixed for: after a real reboot, PID
    numbering resets and climbs from low again - if the OS happens to reassign a launched run's exact old PID
    number to some unrelated process before the backend next checks that run's status, `os.kill(pid, 0)`
    (and the now-fixed zombie check) would both succeed, and the run would be wrongly reported `"running"`
    forever - not "not detected as dead," but actively misattributed to a stranger process. **Deliberately
    not fixed yet** ("note it down somewhere, we worry about it later") - low real-world probability (this
    project's recorded pids tend to be large, accumulated over a long-running WSL2 session; an immediate
    post-reboot collision needs the new PID counter to climb all the way back up to the exact old number,
    not just any number), and this app is confirmed single-user/local (see "Galaxy replacement" project
    memory), so the blast radius of a wrong status label is small. **Real fix, when picked up**: `/proc/
    <pid>/stat` exposes the process's own start time (field 22, `starttime`, in clock ticks since boot) -
    store it alongside `pid` in `run_meta.json` at launch, and have `_pid_alive` (or a renamed
    `_pid_alive_and_matches`) compare the live process's current start time against the recorded one, not
    just whether the pid number currently resolves to *something*. A coincidentally-reused pid will almost
    certainly have a different start time than the run that originally owned it.
...
22. **WGS sibling pipeline (PLAN.md §6.9), Phase 1 (plumbing only, zero real tool stages) — first version silently produced a fake "success," fixed before being reported as working.** Added 2026-09-14, following the owner's directive to build the pathogen/isolate WGS sibling pipeline for real ("option A is more work but more reward"). Phase 1's job is only to prove `main.nf`'s new `params.pipeline = 'metagenomics' | 'wgs'` selector and a new `workflows/wgs.nf` skeleton wire together correctly, before any real WGS tool module exists. **First version tried to always produce a green MultiQC report, even with zero tools enabled** — mirrored `workflows/microbox.nf`'s `Channel.topic('versions')` provenance pattern, but this workflow has no real module anywhere that writes to that topic (MULTIQC itself deliberately does not, per its own module comment), and `MICROBOX`'s modules — which do — are never invoked when `pipeline=wgs`, so they're not live producers either. Real findings, in order: (a) `Channel.topic('versions').collect()` never fired at all — no task was ever submitted, `workflow.success` was `true` regardless, a genuinely confusing silent-nothing failure mode, not an error; (b) switching to a plain `Channel.of(...)` value channel to sidestep that fixed the collectFile writes, but then (c) MultiQC itself exited `0` with **no `multiqc_report.html` written at all** ("No data found in software_versions_mqc.yml" / "No analysis results found. Cleaning up…") — an empty custom-content file is not the same as "no analysis to report," and MultiQC's own behavior for the latter is to produce nothing, not an empty-but-valid report. **This is genuinely the same degenerate case `workflows/microbox.nf` already guards against explicitly** (its own `ifEmpty { error "Nothing enabled to analyze or report..." }` check) rather than a new problem — the first version of this file just hadn't been made to hit that guard yet. **Fix, not a workaround:** `workflows/wgs.nf` now explicitly `error`s with a clear message ("WGS pipeline has no tool stages implemented yet...") whenever it's invoked, and its nf-test case (`tests/main.nf.test`, "wgs: skeleton...") was rewritten to assert `!workflow.success` + the specific message, the same negative/guard-test convention already used by the "everything optional off" and "skip_quast=true" degenerate-case tests above. **Deliberately not left as a fake pass**: the honest state of Phase 1 is "plumbing proven, zero tool stages exist yet," and the test now says exactly that rather than a green checkmark implying more than is true. Full suite (14 tests, including this one) verified green with Docker running for real, not skipped/mocked. This test is a placeholder, replaced by a real positive assertion once Phase 2 (BWA-MEM2) lands.
    **Superseded by Phase 2 (entry 23 below), 2026-09-14, same day:** the explicit `error "WGS pipeline has no tool stages implemented yet..."` described above was replaced by real stage wiring + the standard `ifEmpty { error "Nothing enabled to analyze or report..." }` guard (matching `workflows/microbox.nf`'s own pattern exactly) once Phase 2 gave the workflow a real, skippable stage to guard around — the *test name* referenced above ("wgs: skeleton...") no longer exists verbatim either, replaced by "wgs: defaults..." and a new positive test. Not rewriting this entry's history, just flagging the exact wording is now stale so a future reader isn't confused hunting for it.

23. **WGS sibling pipeline (PLAN.md §6.9), Phase 2 — first real tool stage: BWA-MEM2 reference-guided alignment + samtools stats.** Added 2026-09-14, immediately following Phase 1. Installed via `nf-core modules install bwamem2/index`/`bwamem2/mem` (re-verified live, not assumed from PLAN.md §9.3's 2026-09-11 research — no stale-tag surprise this time, both pin `community.wave.seqera.io/library/bwa-mem2_htslib_samtools:db98f81f55b64113`, a bundled bwa-mem2+htslib+samtools image). **Real findings:**
    1. **`bwamem2/mem` already bundles `samtools sort`/`view` internally** (same pattern as this repo's existing Bowtie2 module) — confirmed by reading the installed module's `script:` block before assuming it, exactly per this repo's own "read the module before assuming" discipline; no separate `samtools/sort`/`samtools/index` module needed. It does NOT auto-produce a BAM index though — that only happens with `--write-index` passed via `ext.args2` (`conf/modules_wgs.config`), needed downstream for `samtools/stats`.
    2. **No native MultiQC module for BWA-MEM2 exists** (verified against `docs.seqera.io/multiqc/modules/` — only a generic `samtools` module) — same "check before assuming" step CONTRIBUTING.md §1 already requires, same outcome as geNomad/CheckV/MaxBin2 needing something extra, except here the "something extra" is a real upstream module (`samtools/stats`, itself installed and wired) rather than a hand-written custom `_mqc.yml` — a cleaner outcome than those three got.
    3. **Toy fixture was first wrongly dismissed as a mechanics-only stand-in, based on an unverified size assumption — corrected the same day after being challenged on it.** Checked `nf-core/bacass`'s own real test fixtures first (the obvious "real bacterial WGS" precedent), found its short-read pair real and reachable but ~115MB total (R1+R2, verified via `HEAD`), and *assumed* — without actually testing it — that this was "too large for a fast `basic`-tagged test," swapping in this repo's own SARS-CoV-2/Lambda-phage mechanics fixture instead. **That assumption was wrong, and was only caught because the owner asked "are you sure about that?"** — actually downloading and aligning it (real `docker run`, not reasoning from file size) showed: ~19s download, BWA-MEM2 index in <1s, alignment of all 1M read pairs in ~20-26s on this profile's 2cpu/6GB ceiling — comparable to or faster than this suite's own existing MaxBin2 test (~80s). **`ERR044595` is real ENA-cataloged *Staphylococcus aureus* sequencing data** (confirmed via ENA's portal API, not guessed); `NC_007795.1` (S. aureus NCTC 8325) is its matching reference — a real run against it maps 90.7% of reads (confirmed against an unrelated reference first, which mapped only 0.15%, ruling out a coincidental high mapping rate). **Fixed, not left as a caveat**: `assets/samplesheet_wgs_test.csv` and `conf/test_wgs.config` now use this real dataset; the Phase 1 "nothing enabled" guard test (which never reads the fastqs at all) keeps the tiny SARS-CoV-2 fixture instead, since realism doesn't matter for a test that fails before touching any file. Lesson for future fixture decisions in this repo: verify feasibility by actually running it, not by eyeballing a file size.
    4. **Real measured resource usage, now on the real bacterial-scale fixture above (Nextflow's own `execution_trace.txt`): `BWAMEM2_INDEX` peak RSS 33.1 MB, `BWAMEM2_MEM` peak RSS 707.9 MB, `SAMTOOLS_STATS` peak RSS 8.2 MB — total pipeline wall time ~1m9s.** Comfortably under the ~7GB dev RAM ceiling for a single sample. **This still doesn't fully resolve the owner's "on by default" request (PLAN.md §6.9)** — it's one sample against a ~2.86 Mb reference; multiple samples running in parallel, or a larger bacterial genome, would use more, and that scaling hasn't been measured — but it's real, meaningful, bacterial-scale evidence now, not the toy-reference number this entry originally reported (a ~48.5kb Lambda-phage reference giving misleadingly tiny 6.9-10.3 MB figures). `skip_bwamem2` stays `true` by default; take these real numbers back to the owner alongside the request, rather than deciding unilaterally either way.
    5. **Cosmetic-but-fixed finding, not left as noise:** a first version chained `.first()` onto `BWAMEM2_INDEX.out.index` (the usual nf-core one-reference-many-samples idiom) and Nextflow warned it was "useless" — `BWAMEM2_INDEX`'s own input is already a `Channel.value()`, so its output is automatically a value channel too (broadcasts to every sample without `.first()`). Removed, re-ran, confirmed the warning is gone and behavior is unchanged (same 4-task run, same outputs) — verified, not just reasoned about.
    Two real nf-test cases added (`tests/main.nf.test`, tag `basic`): the pipeline's own "nothing enabled" degenerate case (now genuinely reachable — `skip_bwamem2` defaults `true` and it's still the only stage) asserting the same guard message `workflows/microbox.nf` uses, and a positive case asserting real content — a real BAM + index exist, `samtools stats`' actual output format appears (`SN\traw total sequences:`), and its data reached MultiQC's aggregated report content (not just that the source file exists on disk, the same class of bug already caught twice before in this repo — entries #13/#22). Full suite (15 tests) verified green with Docker running for real.

24. **WGS sibling pipeline (PLAN.md §6.9), Phase 3 — pre-flight species/contamination screening: Kraken2 (reused) + Mash Screen + seqkit stats.** Added 2026-09-14, immediately following Phase 2. **Scope decided only after being asked to verify real-world usefulness first** ("google to make sure we would be providing something useful, not wasting time") — research found real bacterial WGS QC pipelines (nf-core/bacass, general practice) most commonly use Kraken2 as the primary species/contamination gate, while Mash Screen is also real and field-used but for a more specific signal (fast genome-distance estimation, public-health plasmid-transmission surveillance). **Owner decision: keep both, independently toggleable** ("allow the user to pick whatever they want, as long as they are both functional") rather than a forced either/or. BLAST+ stays deferred (unrelated, prior decision). **Real findings, several found only by actually running things, not by reading docs:**
    1. **Kraken2-WGS reuses the exact same module and DB as the metagenomics pipeline** via an `as`-aliased include (`KRAKEN2_KRAKEN2 as KRAKEN2_KRAKEN2_WGS`, same pattern as the existing `KRAKEN2_KRAKEN2_PREDEPLETION`) — no new module, no new DB param. Tested against the real S. aureus fixture (ERR044595) with only the existing viral-tier Kraken2 DB present: 96.07% unclassified — the correct, honest result for a viral-only DB screening bacterial reads, not a failure to paper over. Real content asserted in the `requires_db`-tagged test.
    2. **`mash/sketch` has NO role in this pipeline, despite being installed for it initially — a real design mistake, caught by actually running the wiring, not by re-reading docs more carefully.** First version sketched the sample's own reads via `MASH_SKETCH` and fed that sketch as `mash screen`'s query. This failed with mash's own `ERROR: reading inputs`, reproduced directly against the real container outside Nextflow to isolate it from any Nextflow-staging red herring. Root cause: `mash screen`'s query argument is meant to be raw sequence files, which mash streams and k-merizes on the fly against the already-loaded reference sketch DB — not a pre-built sketch. **Fixed**: `MASH_SCREEN` now takes `ch_samplesheet` (raw reads) directly as its query; `MASH_SKETCH`'s `include` was removed entirely, not left in unused. Verified against real data: screening ERR044595 against the real RefSeq DB correctly identifies `GCF_000284535.1` (*S. aureus* subsp. *aureus* HO 5096 0412) as a perfect match — identity 1.0, 1000/1000 shared hashes.
    3. **`mash screen` needs `-w` (winner-takes-all) to be usable at all against a 91,282-genome reference DB** — without it, every reference genome sharing even one hash emits its own output row (tens of thousands of near-zero-identity rows for an unrelated 90k+-genome DB), burying the real signal. Added via `ext.args = '-w'` (`conf/modules_wgs.config`), verified directly: with `-w`, the correct top hit is one of a handful of rows instead of one among tens of thousands.
    4. **A real, previously-undetected design bug: Mash had zero MultiQC contribution, which broke the pipeline's own degenerate-case guard for a legitimate, non-degenerate case.** Mash has no native MultiQC module (confirmed against `docs.seqera.io/multiqc/modules`, 182 tools, none named "mash"), and a first version left it that way, reasoning "its own published file is informative enough." This was wrong in a way that only an actual test run surfaced: a run with **only** `skip_mash=false` enabled genuinely ran Mash and produced a real, correct result on disk, but still hit `workflows/wgs.nf`'s `ifEmpty { error "Nothing enabled to analyze or report..." }` guard, because that guard checks `ch_multiqc_files` specifically, not "did any stage run." **Fixed properly, not routed around**: added a real custom MultiQC summary (`mash_screen_mqc.yml`, same `id`/`section_name`/`plot_type: table` custom-content shape already used for `software_versions_mqc.yml`) reporting each sample's top identity/shared-hashes/organism match — this both closes the guard gap and gives the team the top species match directly in the aggregated report instead of requiring them to open `results/wgs/mash/*.screen` by hand. **Second real gap in that same fix, caught by the test's own string assertion failing**: the first summary version captured only the machine-readable accession filename (`GCF_000284535.1_ASM28453v1_genomic.fna.gz`) from column 4 of mash's tab-separated output, not the human-readable organism name in column 5 (`...Staphylococcus aureus subsp. aureus HO 5096 0412...`) — genuinely useless to a person reading the report. Fixed to prefer column 5 when present.
    5. **`seqkit stats` already hardcodes `--tabular` unconditionally in its own script** (`seqkit stats --tabular ... ${args}`) — an early plan draft assumed `ext.args = '--tabular'` needed to be supplied, based on MultiQC's own docs describing that requirement in the abstract; reading the actually-installed module before writing the config found this assumption unnecessary and it was never added. `task.ext.args` defaults to `--all` if unset (also already in the module), giving richer per-file stats (Q20/Q30/AvgQual/GC%/N50) for free.
    6. **Real measured resource usage (Nextflow's `execution_trace.txt`) — the one genuinely concerning number this phase produced**: `KRAKEN2_KRAKEN2_WGS` peak RSS 695.9 MB, `SEQKIT_STATS` peak RSS 32.2 MB, `MASH_SCREEN` peak RSS **6 GB**, `MULTIQC` peak RSS 234.4 MB. Mash's 6GB is right at the edge of this dev machine's real available headroom (`free -h`: 7.5GB total, ~6.5GB available at measurement time) — loading the 754MB on-disk RefSeq sketch DB apparently expands substantially in memory. This is a materially bigger resource risk than Phase 2's BWA-MEM2 findings and should weigh heavily against enabling `skip_mash=false` by default on a machine anywhere near this dev box's ceiling, independent of the still-open general "on by default" question (PLAN.md §6.9).
    7. **A genuinely unrelated, pre-existing environment gap found and fixed while re-verifying, not caused by this phase**: the existing metagenomics `requires_db` test "geNomad + CheckV find and grade the real virus" failed on this machine simply because neither DB had ever been downloaded here. Downloaded both for real (`bin/download-dbs.sh genomad` and `checkv`) at the owner's request ("no harm in that") — geNomad's own test then hit a second, real, pre-existing constraint: its `conf/test.config` override requests 9GB or resourceLimits, but this machine only had ~6.5GB actually available, causing what looks like an OOM-kill (`exit:2147483647`). Left as-is, not touched — outside Phase 3's scope, and changing a shared metagenomics resource override wasn't requested.
    Three real nf-test cases added: `KRAKEN2_KRAKEN2_WGS` and `MASH_SCREEN` tagged `requires_db` (both need pre-downloaded external DBs, same convention as the existing "contigs: Kraken2 alone" test), `SEQKIT_STATS` tagged `basic`. All three assert real observed content, several only decided after actually running the pipeline once to see genuine output rather than an assumed one. Full suite (16 `basic` + 2 of this phase's own `requires_db` cases) verified green with Docker running for real.

25. **WGS sibling pipeline (PLAN.md §6.9), Phase 4 — variant calling: GATK4 HaplotypeCaller (GVCF mode) + GenotypeGVCFs, the final real analysis stage of this build.** Added 2026-09-14, immediately following Phase 3. **Scope decision, owner directive**: always call in `-ERC GVCF` mode then genotype per-sample via `GenotypeGVCFs`, the standard GATK best-practices shape, rather than a plain single-sample VCF — "cover the broadest usecase... whatever they need we had it covered," concretely justified here because this pipeline's own stated scenario (PLAN.md §6.9) is many isolates vs. one reference, and GVCF mode costs nothing extra for one sample while making real cross-sample joint genotyping addable later without recalling anything. True cross-sample combining (`gatk4/genomicsdbimport`/`combinegvcfs`) stays a deliberate, explicit follow-on — building it now would mean an untestable abstraction, since this repo only has one real isolate fixture. **A real fixture simplification found while planning**: rather than sourcing an external "known expected variant" benchmark dataset (originally flagged as a separate open problem in Phase 2's plan), reused the already-verified real ERR044595/NC_007795.1 pair from Phase 2 and ran GATK4 once to see what it actually calls — the same "run it, assert the real observed value" discipline used throughout this build, not a search for an academic gold-truth set. **Three real bugs found and fixed, all by actually running the chain, not by reading docs more carefully:**
    1. **GATK4's `CreateSequenceDictionary` refuses a reference file whose staged filename has no recognized fasta extension** (`java.lang.IllegalArgumentException: File is not a supported reference file type: efetch.fcgi`) — a real, reachable case here, not hypothetical: `params.wgs_reference_fasta` commonly points at an NCBI efetch URL (the same pattern `conf/test.config` already uses for the metagenomics pipeline's `host_fasta`), and those URLs have no filename with an extension, so Nextflow stages the download under the URL's literal CGI script name. BWA-MEM2 tolerates this fine (extension-agnostic); GATK4's htsjdk-based file-type detection does not. **Fixed with a new local module** (`modules/local/rename_reference`, same `modules/local/` pattern as the existing `gzip_contigs`/`maxbin2` - reuses the already-pinned bowtie2 container rather than a new tag for one `cp` call) applied once so every downstream WGS consumer gets a consistently-named `reference.fasta`, not just GATK4's own inputs.
    2. **BWA-MEM2 emits no `@RG` (read group) line by default, which breaks GATK4's GVCF mode outright** with a genuinely confusing error ("Argument emit-ref-confidence has a bad value: Can only be used in single sample mode currently") rather than anything mentioning read groups directly — GATK infers "single sample" from `@RG SM:`, and with none present it can't. **Fixed** with a real `ext.args` read-group (`ID`/`SM`/`LB` set to the sample id, `PL:ILLUMINA` - real for this pipeline's short-read WGS use case) on `BWAMEM2_MEM`, which also means any future multi-sample joint genotyping has real, distinct sample names to key off, not an accident of this fix.
    3. **The genuinely serious one: `GATK4_HAPLOTYPECALLER` and `GATK4_GENOTYPEGVCFS` both defaulted their output to the identical filename (`"${meta.id}.vcf.gz"`), and `GenotypeGVCFs` writing through its own same-named staged input symlink silently corrupted the real GVCF data.** Not a theoretical race - confirmed directly: `HaplotypeCaller`'s own log showed it genuinely processed all 20,331 regions across the whole 2.86 Mb genome in 3m23s real compute, but the file on disk (reached via the shared filename through the symlink) ended up containing `GenotypeGVCFs`' own near-empty output instead, and `GenotypeGVCFs` itself reported "Traversal complete. Processed 0 total variants" despite exiting 0 - a clean-looking success hiding a real data-loss bug. Exact same class of bug as this repo's existing `KRAKEN2_KRAKEN2_PREDEPLETION` filename-collision fix (`conf/modules.config`), same fix shape: a distinct `ext.prefix` (`"${meta.id}.g"`, GATK's own conventional `.g.vcf.gz` GVCF naming) on `GATK4_HAPLOTYPECALLER` so the two stages' outputs never share a name. After the fix: 184,287 real GVCF records, 37,491 real called variants in the final genotyped VCF, including a specific real homozygous SNP confirmed and asserted in the nf-test case (`NC_007795.1:89 C>T`, QUAL 2171.06, DP 53).
    4. **A real nf-test authoring mistake, caught by the assertion actually failing, not assumed correct**: the inapplicable-combination guard test first asserted `workflow.stdout` contains the `log.warn` auto-skip message - it doesn't, because `log.warn` routes through Nextflow's own SLF4J logger (confirmed present in the run's real `.nextflow.log`) rather than the plain `error()`/`println` stream `workflow.stdout` actually exposes to nf-test. Fixed by asserting only the observable behavior (the run still correctly fails with "Nothing enabled to analyze or report," not a silent success) rather than an internal log line nf-test genuinely cannot see.
    5. **Real measured resource usage** (`execution_trace.txt`, real bacterial-scale reference as established in Phase 2, not a toy one): `GATK4_CREATESEQUENCEDICTIONARY` peak RSS 247.5 MB, `GATK4_HAPLOTYPECALLER` peak RSS 776.1 MB (by far the slowest single task, 3m23s real compute for one ~2.86 Mb genome/1M read pairs), `GATK4_GENOTYPEGVCFS` peak RSS 459.7 MB. All comfortably under the ~7GB dev ceiling for one sample - real production-scale runs (larger genomes, many samples, hours-long) are explicitly out of scope for this local verification and are planned for AWS instead (owner, 2026-09-14: "real testing will be done on AWS where resources are not an issue... hitting the RAM limitation is an issue we need to ignore and keep trying" locally).
    Two real nf-test cases added, both tagged `basic` (no external DB needed, unlike Kraken2/Mash - just slower, ~5 minutes for the positive case, mostly `HaplotypeCaller`'s own real per-base computation): a positive case asserting the real called variant above, and the inapplicable-combination guard case. Full suite (18 tests) verified green with Docker running for real, after the earlier memory-pressure interruption (a background nf-test run genuinely got killed for low system memory mid-suite, unrelated to any bug - cleared stray Docker containers and re-ran clean).

26. **Full-UI-architecture Phases 2 and 3 (`~/.claude/plans/jazzy-shimmying-wolf.md`) — real live run status via `-with-weblog`, and Composer's Run button going live for the first time.** Added 2026-09-14, immediately following the WGS pipeline's Phase 4. Phase 2: `bin/run.sh` gained `--weblog-url`/`--extra-params-file`; `server/main.py` gained a `/api/telemetry/{run_id}` receiver and `GET /api/runs/{run_id}` status derivation. Phase 3: the real UI-to-engine converter (`composer-ui/src/utils/pipelineConverter.ts`), WGS tools added to the palette (`toolCatalog.ts`), and Composer's Run button now actually calls `POST /api/runs` instead of being permanently disabled. **Two real bugs found, both only by actually running the system end to end against the live backend, not caught by any of the 30+ unit/component tests written alongside this work:**
    1. **A zombie process reads as "alive."** `server/main.py`'s `_pid_alive` used only `os.kill(pid, 0)` to check whether a launched run's process was still running. A genuinely finished run's launching `bash bin/run.sh` process becomes a zombie once it exits (this backend launches it fire-and-forget via `subprocess.Popen` and never calls `.wait()`/`.poll()` on it) - and a zombie still answers `os.kill(pid, 0)` successfully, since its PID stays in the process table until reaped. Found by driving a real launch through the live server (`curl -X POST /api/runs` with a deliberately-wrong reference path so the real pipeline failed fast) and watching `GET /api/runs/{id}` report `"running"` forever even after the real Nextflow log showed `Pipeline failed (exit 1)` and a real weblog `"error"` event had already arrived. Confirmed the exact mechanism via `ps -p <pid> -o stat` showing `Zs <defunct>`, not guessed. **Fixed** by having `_pid_alive` additionally read `/proc/<pid>/status`'s `State:` field when `os.kill` succeeds - a zombie's `State: Z` is now correctly treated as not-alive, falling through to the weblog's last-seen event (or the on-disk report) same as any other exited process. Verified twice: a real reproduction in `server/test_main.py` (spawns a genuine `subprocess.Popen(["true"])`, waits for it to actually reach zombie state via `/proc`, confirms the old code would have reported "running"), and re-verified live against the actual server process restarted with the fix, watching the same real run's status flip from `"running"` to the correct `"failed"`.
    2. **`-with-weblog` prints a real deprecation warning on this Nextflow version (26.04.6, this project's own pinned version)** - `WARN: The command line option '-with-weblog' is deprecated - consider enabling this feature by setting 'weblog.enabled=true' in your configuration file`, alongside two more WARNs that `weblog.enabled`/`weblog.url` aren't recognized config options (since neither is set in `nextflow.config`). **Confirmed functionally harmless, not glossed over**: the same live smoke test's `weblog_events.jsonl` shows real `started`/`completed`/`error` events actually arrived despite every warning. Left as-is (the CLI flag still works) rather than migrating to the config-based form pre-emptively - noted here as a real, reproducible warning worth knowing about if a future Nextflow upgrade ever removes the deprecated flag outright, not a currently-broken feature.
    Real verification beyond unit tests: 22/22 backend pytest tests (was 12 at the end of Phase 1), 115/115 frontend vitest tests (was 97), ruff/oxlint clean, real production `tsc -b && vite build` clean (which itself caught a real regression - the History page's Refresh button was passing its click `MouseEvent` as the new silent-poll boolean parameter, since fixed). A full live end-to-end smoke test (real backend booted, real `POST /api/runs` via curl, real Nextflow invocation, real params YAML on disk, real weblog events received) is what surfaced both bugs above - kept as the standing lesson for this phase: the test suites alone did not catch either one.
