...
- **Relationship to what the pipeline engine actually consumes**: the saved file is a UI-layer artifact (canvas layout + human intent); the pipeline itself still runs off Nextflow's own params-file mechanism. Keeping these as two distinct things (the saved pipeline file *produces* a params file, rather than *being* one) is what keeps the engine genuinely GUI-agnostic — the CLI-only path shouldn't need to know the visual format exists at all.
- **A concrete payoff already implied by an earlier ask this session**: this is exactly the mechanism for the "working across multiple machines" need raised earlier (§6.5-adjacent, not written there) — export a pipeline configuration on one PC, import it on another, no re-building the canvas from scratch. If the format is plain JSON (fits this project's whole "everything human-readable, everything in git" ethos already established for configs), a saved pipeline could even be checked into the repo itself as a named, reusable template.
