...
2. **A visual, drag-and-drop pipeline *builder*** — actually dragging/arranging nodes in a GUI, not typing `--skip_x`/editing YAML — **is a different, much bigger thing**, and this description sounds like it's asking for it. This is precisely the composition GUI PLAN.md §6.8 item #12 already flagged as an explicit, owner-acknowledged deviation from the original DolphinNext spec (composition happens via CLI flags + YAML instead; "if visual composition ever becomes a hard requirement, re-evaluate — not planned"). Per the owner's own framing just now ("its a matter of expert, and tool sensitive and need to be studied"): **not scoped, not estimated, not started.** Open questions before it could even be scoped: does "visual" mean a real node-graph editor (essentially rebuilding what DolphinNext/Galaxy/Seqera Platform already are), or a simpler guided form-based flow in the existing Streamlit UI (pick tool A, pick tool B, see the resulting command)? Which parameter combinations are even valid to expose per node (a raw parameter surface per tool is large and mostly not meant for a non-expert to touch directly)? How does "come back another day and edit step X" reconcile with runs already being immutable/hashed by Nextflow's own caching model? None of this is answered yet — deliberately, since it needs domain + tooling expertise this plan doesn't have yet, not a guess baked into the architecture now.
